...
fn calculate_distances(input: &str) -> BTreeMap<(isize, isize), usize> {
    ...
    let mut s_points: BTreeSet<(isize, isize)> = BTreeSet::new();
    let mut e_points: BTreeSet<(isize, isize)> = BTreeSet::new();
    ...
    while let Some(c) = chars.next() {
        match c {
            ...
            '$' => break,
            '(' => {
                positions_stack.push((s_points, e_points));
                s_points = positions.clone();
                e_points = BTreeSet::new();
            },
            ')' => {
                positions.append(&mut e_points);
                let (s, e) = positions_stack.pop().unwrap();
                s_points = s.clone();
                e_points = e.clone();
            },
            '|' => {
                e_points.append(&mut positions);
                positions = s_points.clone();
            }
